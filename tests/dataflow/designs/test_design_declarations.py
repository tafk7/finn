...
"""KD3: ``DataflowDesign`` owns semantics and ``KernelChoice`` specializes ``SubspaceChoice``."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda name: NetworkEdge(
            TwoSegments.first.output("output"),
            EdgeSink(TwoSegments.second.input("input")),
            name=name,
        ),
        lambda name: NetworkBoundary(TwoSegments.first.input("input"), name=name),
    ],
...
def test_none_is_the_only_specialized_design_name_fallback() -> None:
    ...
    assert NetworkBoundary(TwoSegments.first.input("input")).stable_name is None
