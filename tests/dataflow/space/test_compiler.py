...
from finn.dataflow.space.compiler import _Ref, _compile_space, compile_space
...
def test_frontend_remains_domain_free() -> None:
    root = Path(__file__).parents[3] / "src/finn/dataflow/space"
    #: The generic core.  ``dataflow_value_semantics`` is deliberately absent:
    #: it is the one declared bridge module, and knowing both sides is its job.
    files = (
        root / "__init__.py",
        root / "branching.py",
        root / "compiler.py",
