...
EXPECTED_MANIFEST: tuple[tuple[str, str, str], ...] = (
    ("compute.finnlib.0", "finnlib", "rtl/infra/replay_buffer.sv"),
    ("compute.finnlib.1", "finnlib", "rtl/arith/add_multi_pkg.sv"),
    ("compute.finnlib.2", "finnlib", "rtl/arith/add_multi.sv"),
    ("compute.finnlib.3", "finnlib", "rtl/linalg/dotp_8sx9_dsp58.sv"),
    ("compute.finnlib.4", "finnlib", "rtl/linalg/dotp.sv"),
    ("compute.finnlib.5", "finnlib", "rtl/linalg/dotp_axi.sv"),
)
...
#:
#: **The FinnLib ReplayBuffer migration moves source identity and the physical
#: component name, and nothing semantic.** The replay Kernel keeps its id,
#: parameters, Region, ABI, and placement. Its source root/path and component
#: module change from FINN to FinnLib, so the component fingerprint and every
#: source-derived top/artifact name move. The generated wrapper also updates
#: one ownership comment. Numeric/control interfaces, connections, boundaries,
#: and parameter tables must remain byte-identical.
#:
...
PRE_MIGRATION_TOP_MODULE_NAMES = {
...
@pytest.mark.parametrize("config", fixture.CONFIGS, ids=lambda item: item.label)
def test_only_source_ownership_moves_in_the_elaborated_hardware(
    config: fixture.Config,
) -> None:
    """The source migration changes provenance, not interfaces or behavior."""

    built = fixture.decomposed_requirements(config)
    actual = _physical(built)
    baseline = BASELINE[config.label]["physical"]
    moved = {"components", "associations", "wrapper"}
    assert {key: value for key, value in actual.items() if key not in moved} == {
        key: value for key, value in baseline.items() if key not in moved
    }
    assert all(actual[key] != baseline[key] for key in moved)


@pytest.mark.parametrize("config", fixture.CONFIGS, ids=lambda item: item.label)
def test_only_the_source_derived_build_name_moves(config: fixture.Config) -> None:
    """Target and timing stay fixed while the source-derived top name moves."""

    built = fixture.decomposed_requirements(config)
    actual = _build_inputs(built)
    top = str(actual["top_module_name"])
    assert top != PRE_MIGRATION_TOP_MODULE_NAMES[config.label]
    assert actual["wrapper_file_name"] == f"{top}.sv"
    assert actual["target_fpga_part"] == config.fpga_part
    assert actual["clock_period_ns"] == fixture.CLOCK_PERIOD_NS
...
def test_the_generated_wrapper_changes_only_its_source_derived_name() -> None:
    """The readable wrapper stays exact apart from its source-derived name."""

    built = fixture.decomposed_requirements(fixture.CONFIGS_BY_LABEL["softvec"])
    expected = SOFTVEC_WRAPPER.replace(
        PRE_MIGRATION_TOP_MODULE_NAMES["softvec"], built.top_module_name
    )
    assert built.wrapper_source == expected
