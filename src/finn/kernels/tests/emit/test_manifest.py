...
"""ArtifactManifest single-source-of-truth + the .abc reader (M4).

A manifest's three reader-views (filenames / static_files / resources) stay consistent,
the in-tree bundles read their source list FROM the manifest (so Backend.sources and the
emit's StaticFile list cannot drift), and the ``.abc`` reader resolves finnlib dependency
manifests transitively — dep-first, de-duped, raising on a missing component.
"""
...
def test_source_file_resolves_its_path():
    ...
    assert sf.as_static() == StaticFile(
        "finn.data", "finn-rtllib/thresholding/hdl/thresholding.sv"
    )
...
def test_manifest_reader_views_are_consistent():
    m = ArtifactManifest(
        sources=(SourceFile("a.sv", root="dir1"), SourceFile("b.sv", root="dir2")),
        top="a_top",
    )
...
def test_decoupled_bundle_sources_come_from_manifest():
    from finn.kernels.dataflow.parameters.emit_memstream import MEMSTREAM_MANIFEST
    from finn.kernels.dataflow.parameters.impl_decoupled import decoupled_topology
