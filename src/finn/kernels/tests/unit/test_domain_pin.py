...
"""Domain-pin guard (PERMANENT — keep past Phase 2).
...
op MOVE (a file relocation, an ``__init__`` import edit) can silently break that resolution
without any other test noticing — the failure only surfaces deep in a build flow. This guard
asserts the pin holds: every registered op resolves from the domain and instantiates.
"""
...
def test_custom_op_dict_is_populated():
    """The domain pin exposes exactly the registered ops, each mapped to a KernelOp class."""
...
def test_domain_resolves_and_instantiates(op_type):
    """A ``(domain="finn.kernels", op_type)`` node resolves through qonnx's getCustomOp to
    the pinned class and instantiates — the exact path the build flow's post-infer passes
    take. A committed-config getter (``onnx_opset_version``) is exercised to prove the
    instance is live, without needing a full model."""
    ...
    # A context-free getter must answer bare-node (Group-1 committed-config contract).
