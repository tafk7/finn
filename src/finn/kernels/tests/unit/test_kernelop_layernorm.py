...
def _layernorm_op() -> Kernel:
    ...
    return Kernel(
        identity=KernelSchema(
            name="LayerNorm",
            interfaces=(
                InterfaceSchema("inp", Direction.IN, block=list(channel_block)),
...
def test_configures_to_a_point():
    op = _layernorm_op()
    pt = op.configure(_ctx(), {"backend": HLS, "SIMD": 16})
    assert not isinstance(pt, Illegal)
    assert pt["SIMD"] == 16
    assert pt["backend"] == "layernorm_hls"
...
def test_folded_input_shape_matches_finn(simd):
    op, ctx = _layernorm_op(), _ctx()
    pt = op.configure(ctx, {"backend": HLS, "SIMD": simd})
...
def test_bad_index_raises():
    op, ctx = _layernorm_op(), _ctx()
    pt = op.configure(ctx, {"backend": HLS, "SIMD": 16})
...
def test_non_dividing_simd_is_illegal_at_resolve():
    ...
    op = _layernorm_op()
    result = op.configure(_ctx(), {"backend": HLS, "SIMD": 5})
    assert isinstance(result, Illegal)
