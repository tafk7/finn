...
from finn.codegen.CG_backend_registration import CG_BackendRegistry
...
class CodegenValidator:
    ...
    def __init__(self, enable_clean_backends: bool = True, fallback_to_legacy: bool = True):
        ...
        self.clean_registry = CG_BackendRegistry()
        if enable_clean_backends:
            self.clean_registry.enable_clean_backends()
        
        self.legacy_registry = CG_BackendRegistry()
        self.legacy_registry.disable_clean_backends()
    ...
    def _get_clean_backend(self, operation_type: str, backend_type: str):
        ...
        try:
            return self.clean_registry.get_backend_class(operation_type, backend_type)
        except Exception as e:
    ...
    def _get_legacy_backend(self, operation_type: str, backend_type: str):
        ...
        try:
            return self.legacy_registry.get_backend_class(operation_type, backend_type)
        except Exception as e:
    ...
    def _generate_with_metrics(self, backend, test_model, **kwargs) -> Tuple[str, ValidationMetrics]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if hasattr(backend, 'generate_code'):
                    generated_code = backend.generate_code()
                elif hasattr(backend, 'generate_hls_code'):
                    generated_code = backend.generate_hls_code()
                elif hasattr(backend, 'generate_rtl_code'):
                    generated_code = backend.generate_rtl_code()
                else:
                    # Fallback - try to get template values and render
                    template_name = backend.get_template_name()
                    template_values = backend.get_template_values(template_name)
                    generated_code = backend.template_engine.render_template(template_name, template_values)
