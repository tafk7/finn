...
def _input_path(item: RegionInput) -> str:
    ...
    unchanged.  An internal one has no port id to be named by, and is addressed
    ...
    return (
        ...
        else f"internal_input[{item.operand.id!r}]"
    )
...
def validate_region(region: DataflowRegion) -> RegionValidationReport:
    ...
    interfaces that exist, so an internal input contributes to none of them.
    ...
    # port-bearing interfaces participate: an internal input has no port id to
    ...
    # One input per operand.  An internal input has no port id to distinguish a
    ...
    # the internal-input case existed there was nothing here to check.
