...
"""Elaborate whichever compute Kernel the design point selected.
...
#: Compute Kernel id -> the code that realizes it, for members that have moved
#: to the physical Kernel layer.  This table is the whole of the new path: no
#: provider id, no inventory lookup, no second selectable identity.
MVAU_HARDWARE_ELABORATORS: Mapping[str, MVAUElaborator] = MappingProxyType(
    {MVAUComputeKernelId.DOT_PRODUCT.value: elaborate_decomposed}
)
...
def elaborate_mvau(resolved: MVAUResolvedDesign) -> MVAUPhysicalElaboration:
    ...
    migrated = MVAU_HARDWARE_ELABORATORS.get(kernel_id)
    if migrated is not None:
        return migrated(resolved)
